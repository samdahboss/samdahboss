# 👋 Hi there, I'm WumiCodes! 

Welcome to my corner of GitHub, where creativity meets code! I’m a **Full-stack Developer** and a Software Engineering student on a mission to build helpful, scalable software solutions. Check out my Portfolio Website at: [WumiCodes](https://wumi.codes)

<a href="https://www.linkedin.com/in/WumiCodes"><img src="https://img.shields.io/badge/Open_to_Work-Software_%7C_Frontend_%7C_FullStack-brightgreen?style=for-the-badge&logo=linkedin" alt="Open to Work" /></a>



<p align="center">
  <img align="center" src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExb2szcTZ1MjV1cmN1bHpqcnRnaXNidnV5cGhrd3Zrejlsd2Rwc3dkaSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/MYI6NK4JOGpOzOriEg/giphy.gif" alt="coding gif" width="400" />
</p>

## Skills & Technologies
[![My Skills](https://skillicons.dev/icons?i=react,next,vue,express,nodejs,typescript,js,html,css,tailwind,bootstrap,laravel,mongodb,mysql,postgres,redux,git,github,vscode,figma&theme=dark)](https://skillicons.dev)

## Highlights
- **Education**: Final-year BSc. Software Engineering student (First Class standing) at Mountain Top University.
- **Experience**: Full-stack Developer Intern at **Edutams** and Frontend Developer at **TekkTopia Ltd**.
- **Expertise**: Comprehensive experience with frontend (React, Next.js, Vue), backend (Node.js, Express, Laravel), and database management (SQL & NoSQL).
- **Leadership**: Project Director at ATC MTU and Techadem Tech Team Lead.

## Featured Projects
- **[Stockeep Inventory Manager](https://stockeep.com)**: An inventory and store management system built with PostgreSQL, React, and Node.js to monitor inventory levels and track product movement in real-time.
- **[VendorBoost Marketing Platform](https://salesbooster.vercel.app)**: A marketing automation tool built on the MERN stack (MongoDB, React, Node.js), allowing small-scale vendors to leverage WhatsApp networks to share products collaboratively.

## Certifications
- **IBM Certified Software Engineer** (March 2024)
- **Fullstack Web Development Certificate** — Udemy (June 2025)
- **Certified JavaScript & Web Developer** — FreeCodeCamp & Johns Hopkins University (2024)

## I’m currently:
- Working on full-stack applications and converting complex Figma designs into production-ready code.
- Wrapping up my Software Engineering degree at Mountain Top University.

## Let's Build Together! I'm currently looking for:
- **My next big role:** Actively interviewing for Full-stack or Frontend Engineering positions.
- **Freelance clients:** Helping businesses build their digital presence, inventory systems, or SaaS platforms.
- **Open Source:** Contributing to tools that help other developers build faster.

## How to reach me:
- **Email**: [samuelolawumi65@gmail.com](mailto:samuelolawumi65@gmail.com)
- **LinkedIn**: [linkedin.com/in/WumiCodes](https://www.linkedin.com/in/WumiCodes)
- **Phone Number**: +234 70 6809 0383

<br>

<div align="center">
  <h3>Show some ❤️ by starring some of the repositories!</h3>
</div>
